...
# Create SSH Key
$S_DIR_PATH/ft-util/ft_util_sshkey ${app_user}

# Setup of authorized_keys
$S_DIR_PATH/ft-util/ft_util_sshauth ${app_user}
...
echo "${app_user} ALL=(ALL) NOPASSWD:${bin_dir}/osync.sh /usr/local/etc/${app_name}.conf --silent" | sudo EDITOR='tee -a' visudo --file=$sudoers_etc &>/dev/null
...
$S_DIR/ft-util/ft_util_conf-update -s "$S_DIR/etc/${app_name}.conf" -d "/usr/local/etc/${app_name}.conf"


INITIATOR_SYNC_DIR="/var/lib/samba/sysvol"
TARGET_SYNC_DIR=

ssh://ft-osync-sysvol@${other_dc_fqdn}:22//var/lib/samba/sysvol
